...
global $APPLICATION;
if(in_array('print',array_keys($_GET))) { include($_SERVER['DOCUMENT_ROOT'].$APPLICATION->GetTemplatePath("components/bitrix/form.result.view/newform/print.php")); exit; }
...
<?if ($arParams["F_RIGHT"] >= 25) { ?><a href="http://<? echo $_SERVER['HTTP_HOST'].$APPLICATION->GetCurUri("print"); ?>" target="_blank" class="feedback_print" title="Распечатать обращение">Печать</a><? } ?>
<p>
		...
		************************************************/
		...
		if(count($arResult["arrResults"]) > 0)
		{
			...
			if ($arParams["SHOW_STATUS"]=="Y" || $arParams["can_delete_some"] && $arRes["can_delete"])
			{
				...
				{
			?>
				<tr><td colspan="<?=$colspan?>" class="form-results-delimiter" style="background-color:#D8E2E8; height:10px;"></td></tr>
			<?
				} //endif ($j>1);
			?>
				<tr>
