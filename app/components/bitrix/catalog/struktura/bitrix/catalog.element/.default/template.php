...
<?
   ...
   if(is_array($arOfficer) && $arResult["PROPERTIES"]["STR_OFFICER"]["VALUE"] && strlen($arOfficer["~NAME"]) > 0){
	if (is_array($arResult["DETAIL_PICTURE"])){?>
		<div class="news_img" style="width: <?=$arResult["DETAIL_PICTURE"]["WIDTH"]?>; float: left;"><img src="<?=$arResult["DETAIL_PICTURE"]["SRC"]?>" width="<?=$arResult["DETAIL_PICTURE"]["WIDTH"]?>" height="<?=$arResult["DETAIL_PICTURE"]["HEIGHT"]?>" hspace="15" vspace="2" align="left" border="0"></div>
<?	}
	elseif(is_array($arOfficer) && $arResult["PROPERTIES"]["STR_OFFICER"]["VALUE"] && $arOfficer["DETAIL_PICTURE"]){
		...
		?>
		<div class="news_img" style="width: <?=$arFile["WIDTH"]?>; float: left;"><img src="<?=$filename?>" width="<?=$arFile["WIDTH"]?>" height="<?=$arFile["HEIGHT"]?>" hspace="15" vspace="2" align="left" border="0"></div>
<?	}else{?>
		<div class="news_img" style="width: 150; float: left;"><img src="/bitrix/templates/AMS/images/no_photo.gif" width="150" height="200" hspace="15" vspace="2" align="left" border="0"></div>
<?}?>
		<font class="news" style="font: bold 16pt/11pt Arial, sans-serif;"><?=$arOfficer["~NAME"]?><br></font><br style="font: normal 2pt/2pt Arial, sans-serif;">
<?   }
	if(strlen($arResult["PROPERTIES"]["STR_POSITION"]["VALUE"]) > 0){?>
		<div style="padding: 10px;"><font class="news_text"><b><?=$arResult["PROPERTIES"]["STR_POSITION"]["VALUE"]?></b></font></div>
<?	}
