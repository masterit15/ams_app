...
//ДЛЯ ИЗМЕНЕНИЯ [ORIGINAL_NAME] на ДАТУ СОЗДАНИЯ ФАЙЛА ---- end
			...
			?>
				<?//PR($arFile);?>
				<a class="data_file" href="<?=$filename?>">
				<i class="fa <?=$ICON_PATH;?>" aria-hidden="true"></i><?if($arProperty["DESCRIPTION"])echo $arProperty["DESCRIPTION"]; else echo $arProperty["NAME"];?>
				<i class="fa fa-calendar"></i><?=$arFile['TIMESTAMP_X'];?>
				<i class="fa fa-download"></i><?=byteCount($arFile["FILE_SIZE"])?></a>
		<?
		}
	  }
   }
...
//-----------------------------
  ...
  <tr><td align="left" valign="bottom" style="padding: 10px 10px 10px 24px;"><?
   if ($elementID==$arElement["ID"]) showFiles($arParams["IBLOCK_ID"], $arElement["ID"]); else { ?><a href="?ELEMENT_ID=<?=$arElement["ID"]?>#<?=$arElement["ID"]?>" title="Показать подробности">Подробнее...</a><?}?></td><td align="right" valign="bottom"><font class="small_news_date" color=#7D7D78><b><?
