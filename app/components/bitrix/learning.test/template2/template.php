...
<?php else:?>
	...
	<?elseif ($arResult["TEST_FINISHED"] === true):?>
		...
		<a href='protocol.php' target='_blank'>Протокол тестирования для распечатки</a>
		<br>
		<? 
		
			$arrOtvSort = CTestResult::GetList(
                Array("QUESTION_ID" => "ASC"),
                Array("CORRECT" => "N", "ATTEMPT_ID" => $arResult["ATTEMPT"]["ID"], "ANSWERED" => "Y")
			);//получаем список ответов
 
			$arrOtv = $arrOtvSort->GetNext();
				   
			if($arrOtv){
					echo "Неправильно даны ответы на вопросы: <ul>";
			}
				   
			while($arrOtv){
					echo "<li>".($arrOtv["DESCRIPTION"]?$arrOtv["DESCRIPTION"]:$arrOtv["QUESTION_NAME"])."</li>";
					$arrOtv = $arrOtvSort->Fetch();
			}
			...
			$_SESSION[test][result]= $arResult["ATTEMPT"];
		?>
		
		<? 
			/*
			//Запишем в сессию результаты
			...
			$arrOtv = $arrOtvSort->GetNext();
			$_SESSION[test][incorrect]=array();
			while($arrOtv){
				$_SESSION[test][incorrect][]=$arrOtv["QUESTION_NAME"];
				$arrOtv = $arrOtvSort->Fetch();
			}
